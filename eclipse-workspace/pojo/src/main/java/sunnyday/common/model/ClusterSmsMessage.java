...
public class ClusterSmsMessage  implements Serializable, Cloneable{
	...
	public ClusterSmsMessage(SmsMessage sms){
		this.sn=sms.getSn();
		this.submit_sn=sms.getSubmit_sn();
		this.user_sn=sms.getUser_sn();
		this.user_id=sms.getUser_id();
		this.service_code=sms.getService_code();
		this.ext_code=sms.getExt_code();
		this.user_ext_code=sms.getUser_ext_code();
		this.td_code=sms.getTd_code();
		this.sp_number=sms.getSp_number();
		this.filter_flag=sms.getFilter_flag();
		this.mobile=sms.getMobile();
		this.msg_content=sms.getMsg_content();
		this.msg_id=sms.getMsg_id();//用户提交时的msgid
		this.insert_time=sms.getInsert_time();
		this.update_time=sms.getUpdate_time();
		this.status=sms.getStatus();
		this.response=sms.getResponse();
		this.fail_desc=sms.getFail_desc();
		this.tmp_msg_id=sms.getTmp_msg_id(); //不同通道的临时msg_id
		this.stat_flag=sms.getStat_flag();
		this.sub_msg_id=sms.getSub_msg_id();
		this.pknumber=sms.getPknumber();
		this.pktotal=sms.getPktotal();
		this.price=sms.getPrice();
		this.charge_count=sms.getCharge_count();
		this.msg_format=sms.getMsg_format();
		this.err=sms.getErr();
		this.dest_flag=sms.getDest_flag();//流量走向标识
		this.msg_receive_time=sms.getMsg_receive_time();//接收端收到的时间
		this.msg_deal_time=sms.getMsg_deal_time();//dealdata处理开始时间
		this.msg_scan_time=sms.getMsg_scan_time();//发送端扫描时间
		this.msg_send_time=sms.getMsg_send_time();//数据发送时间
		this.msg_report_time=sms.getMsg_report_time();//状态报告返回时间
		this.check_user = sms.getCheck_user();
		this.cache_sn=sms.getCache_sn();
		this.country_cn=sms.getCountry_cn();//国家中文名称
		this.ori_mobile=sms.getOri_mobile();//原始手机号码
//		this.signature=sms.getSignature();//签名信息
//		this.operator_signature =sms.getOperator_signature();
//		this.istest =sms.getIstest();//是否是从Test表过来的数据
		this.area_code=sms.getArea_code();
		this.complete_content =sms.getComplete_content();
		this.report_fail_desc =sms.getReport_fail_desc();
		this.try_times=sms.getTry_times();
		this.md5_index=sms.getMd5_index();
		this.src_number=sms.getSrc_number();
		this.rpt_seq=sms.getRpt_seq(); //状态报告和下发历史的唯一关联标识
	}
	...
	public void addExtraField(String key, Object value){
		...
		if(isKeyOK && isValueOK){
			if(extraFields == null){
				extraFields = new ConcurrentHashMap<String, Object>();
			}
	...
	public Object clone(){
		ClusterSmsMessage result = null;
		try {
			result = (ClusterSmsMessage)super.clone();
