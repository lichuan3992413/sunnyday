 ...
 */
@Service
public class TemplateSmsServiceImpl implements ISmsService{
	private static Logger log = CommonLogFactory.getCommonLog(TemplateSmsServiceImpl.class);
	public HSResponse doSomething(CommonParameter common_param) {
		long time0 = System.currentTimeMillis();
		long time1 = System.nanoTime();
		StringBuffer sb = new  StringBuffer();
		SendSmsRequest request = (SendSmsRequest) common_param.getOther().get("SendSmsRequest");
		...
		HSResponse base = checkTemplate(request);
		long time2 = System.nanoTime();
		sb.append("\n模板验证:　").append(time2-time1).append(" ns").append("\n");
		...
		sb.append("业务验证耗时:　").append(time3-time2).append(" ns").append("\n");
		if(info!=null){
		...
		sb.append("内容追加:　").append(time4-time3).append(" ns").append("\n");
		List<SubmitBean> smsList  = new ArrayList<SubmitBean>();
		Set<String> mobiles = request.getMobiles();
		if(mobiles!=null&&mobiles.size()>0){
			SubmitBean bean = null;
			for(String mobile:mobiles){
				bean = new SubmitBean();
				String[] tmp_mobiles ={mobile};
				bean.setContent(tmp_content);
				bean.setUser_id(corp_id);
				bean.setMobiles(tmp_mobiles);
                bean.setMsg_id(""+common_param.getOther().get("msgID"));//msg_id
...
//		}
		...
		sb.append("其他验证:　").append(time5-time4).append(" ns").append("\n");
		int count = 0 ;
		for(SubmitBean resultBean:smsList){
			...
			//定时下发
			if(common_param.getOther().containsKey("timing_time")){
				resultBean.addExtraField("timing_time", common_param.getOther().get("timing_time"));
			}
			...
			sb.append("计费验证:　").append(time6-time5).append(" ns").append("\n");
			...
			sb.append("错误解析:　").append(time7-time6).append(" ns").append("\n");
		...
		sb.append("总耗时:　").append(time).append(" ms").append("\n");
		sb.append("--------------------------------").append("\n");
		if(time>150){
			LogUtil.getLog().info(sb.toString());	
		}
