...
import sunnyday.gateway.util.ControlModel;
import sunnyday.gateway.util.FileUtil;
import sunnyday.gateway.util.MyClassLoader;
import sunnyday.tools.util.CommonLogFactory;

@Service
public class ControlThread extends Thread implements Stoppable{
	...
	public void run(){
		...
		while (run) {
			try{
				List<ThreadControllerForm> list = rao.getNewThreadControllerInfoByKey("threads:*");
				//monitor_log.info("ThreadControllerForm:  "+list);
				if(list!=null&&list.size()>0){
					controllerFormMap.clear();
					for(ThreadControllerForm form: list){
						String key=form.getServer_ip();
						if(key==null||"".equals(key)){
							monitor_log.warn("不合法服务器IP数据: "+form);
							continue;
						}
